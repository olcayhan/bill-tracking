...
export default function ShowAddAnnouncementFormModal({ show, handleClose }) {
    ...
    const handleSubmit = useCallback(() => {
        addAnnounces({ message: message, localDate: new Date().toLocaleDateString(), date: Date.now() })
    ...
    return (
        <Modal show={show} onHide={handleClose} >
            ...
            <Modal.Body className="">
                ...
                <button className='btn btn-success ms-auto m-3 w-100' onClick={handleSubmit}>Gönder</button>
                ...
                <div className='flex flex-col'>
                    {
                        announces?.length !== 0 ?
                            announces.map((announce, key) => {
                                return (
                                    <AnnounceItem key={key} data={announce} />
                                )
